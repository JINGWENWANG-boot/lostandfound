...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-popconfirm title="Confirm delete?" @confirm="confirmDelBatch" v-if="auths.includes('recruit.deleteBatch')">
        <template #reference>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="Classification"><template #default="scope"><span v-if="scope.row.category_id">{{ state.categoryOptions.find(v => v.id === scope.row.category_id) ? state.categoryOptions.find(v => v.id === scope.row.category_id).name : '' }}</span></template></el-table-column>
      ...
      <el-table-column label="Publisher"><template #default="scope"><span v-if="scope.row.user_id">{{ state.userOptions.find(v => v.id === scope.row.user_id) ? state.userOptions.find(v => v.id === scope.row.user_id).name : '' }}</span></template></el-table-column>
        ...
        <el-table-column label="operate" width="220">
          <template #default="scope">
            ...
            <el-popconfirm title="Confirm delete?" @confirm="del(scope.row.id)" v-if="auths.includes('recruit.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="Lost and found information" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="125px" style="padding: 0 20px" status-icon>
        <el-form-item prop="categoryId" label="Classification">
